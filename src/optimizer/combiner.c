...
/*   Updated: 2023/03/28 05:24:57 by htsang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "PUSH_SWAP/optimizer.h"

static t_push_swap_optimize_possible	optimizer_combiner_init(\
...
t_push_swap_instruction_list *instruction_list)
{
	...
	if (!instruction_list->next || !instruction_list->next->next || \
		optimizer_combiner_init(combiner, instruction_list->next->instruction))
		return (NOTHING_TO_OPTIMIZE);
