...
/*   Updated: 2023/03/24 18:54:43 by htsang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_push_swap_emulation_priority_location forced_search)
{
	...
	return (move_priority_elements(instructor, triangle_maker, \
		priority_amount - 1, forced_search));
}
...
t_push_swap_triangle_maker *triangle_maker, unsigned int priority_amount)
{
			...
			get_emulation_priority_by_index(0, triangle_maker)))
	{
		if (emulate_instruction(instructor, triangle_maker, SA))
			return (FAILURE);
		return (move_priority_elements(instructor, triangle_maker, \
			priority_amount, PRIORITY_AT_FRONT));
	}
...
t_push_swap_triangle_maker *triangle_maker, unsigned int priority_amount)
{
	if (emulate_instruction(instructor, triangle_maker, RRA))
		return (FAILURE);
	return (move_priority_elements(instructor, triangle_maker, \
		priority_amount, PRIORITY_AT_REAR));
}
...
t_push_swap_emulation_priority_location forced_search)
{
	...
	return (move_priority_elements(instructor, triangle_maker, priority_amount, \
		get_priority_location(triangle_maker, triangle_maker->highest_priority)));
}
